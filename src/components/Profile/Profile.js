...
function Profile({ onProfileUpdate, onSignout, serverResponse}) {
  ...
  const buttonsMarkup = () => {
    if (readOnly) {
      return (
        <>
          <button className="profile__button profile__button_edit" onClick={toggleProfileReadOnlyChange} type='button'>Редактировать</button>
          <button className="profile__button profile__button_logout" onClick={onSignout} type='button'>Выйти из аккаунта</button>
        </>)
    }
    return (
      <button className='profile__button-save' disabled={!(profileChanged && isFormValid)} type='submit'>Сохранить</button>
    )
  }
  ...
  return (
    <section className='profile'>
      ...
      <main className='profile__wrap'>
        <h2 className='profile__title'>{`Привет, ${currentUser.name || 'пользователь'}!`}</h2>
